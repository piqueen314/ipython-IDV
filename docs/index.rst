.. ipython-IDV documentation master file, created by
   sphinx-quickstart on Mon Dec 11 13:08:59 2017.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.


.. toctree::
   :maxdepth: 2
   :hidden:

   installguide
   devguide
   examplepage



DRILSDOWN Project
==================

DRILSDOWN brings the GUI-driven 3D geospatial data visualization power of Unidata's IDV into the world of Jupyter notebooks.

It also envisions a higher-level object, a "Case Study", comprising a notebook.ipynb, an IDV bundle.zidv, images, and anything else.

Finally, we have a repository structure (in Geode Systems' RAMADDA system) for these Case Study objects, with services.

As a result, the project's materials are not just pure Python: IDV and RAMADDA are Java applications.

User: please choose your level of interest:
1. I want to use IDV only (some plugins you can find here will facilitate its use).
2. I also want IDV to interact with Jupyter
3. I also want my work to interact with a RAMADDA repository.
4. I also want to install a RAMADDA of my own!

